...
public class CvUtils {
    ...
    //Mat в бинарный файл с расширением .mat
    //Можно сделать так, что указываешь файл, и туда сохраняется новый файл(твоя матрица) с расширением .mtcv
    public static boolean saveMat(Mat m, String path) {
        if (m == null || m.empty()) return false;
        if (path == null || path.length() < 5 || !path.endsWith(".mtcv"))
            return false;
        if (m.depth() == CvType.CV_8U) {}
